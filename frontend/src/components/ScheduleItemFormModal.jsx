...
const ScheduleItemFormModal = ({ isOpen, onClose, onSubmit, item }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-gray-800 p-6 rounded-lg shadow-xl w-full max-w-md">
        <h2 className="text-2xl font-bold mb-4 text-white">{item ? 'Edit Schedule Item' : 'Add Schedule Item'}</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end space-x-4">
            ...
            <button type="submit" className="px-4 py-2 rounded-md text-white bg-indigo-600 hover:bg-indigo-700">{item ? 'Save Changes' : 'Add Item'}</button>
          </div>
        </form>
      </div>
    </div>
...
export default ScheduleItemFormModal;
